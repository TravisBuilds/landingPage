...
exports.default = create_testing_library_rule_1.createTestingLibraryRule({
    ...
    create(context, _, helpers) {
        ...
        function reportCandidateModule(moduleNode) {
            if (node_utils_1.isImportDeclaration(moduleNode)) {
                if (node_utils_1.isImportDefaultSpecifier(moduleNode.specifiers[0])) {
                    const { references } = context.getDeclaredVariables(moduleNode)[0];
                    reportImportReferences(references);
                }
                const cleanupSpecifier = moduleNode.specifiers.find((specifier) => node_utils_1.isImportSpecifier(specifier) &&
                ...
                if (node_utils_1.isObjectPattern(declaratorNode.id)) {
                    const cleanupProperty = declaratorNode.id.properties.find((property) => node_utils_1.isProperty(property) &&
                    ...
                    const references = node_utils_1.getVariableReferences(context, declaratorNode);
                    reportImportReferences(references);
                }
            }
        }
