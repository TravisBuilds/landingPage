...
function findClosestCallExpressionNode(node, shouldRestrictInnerScope = false) {
    if (is_node_of_type_1.isCallExpression(node)) {
        return node;
    }
    if (!node || !node.parent) {
        return null;
...
function hasThenProperty(node) {
    return (is_node_of_type_1.isMemberExpression(node) &&
...
function hasChainedThen(node) {
    ...
    if (is_node_of_type_1.isCallExpression(parent) && parent.parent) {
...
function isPromiseAll(node) {
    return (is_node_of_type_1.isMemberExpression(node.callee) &&
...
function isPromisesArrayResolved(node) {
    ...
    return (!!closestCallExpression.parent &&
        is_node_of_type_1.isArrayExpression(closestCallExpression.parent) &&
        is_node_of_type_1.isCallExpression(closestCallExpression.parent.parent) &&
...
function isPromiseHandled(nodeIdentifier) {
    ...
    for (const node of suspiciousNodes) {
        ...
        if (is_node_of_type_1.isArrowFunctionExpression(node.parent) ||
            is_node_of_type_1.isReturnStatement(node.parent)) {
            return true;
        }
...
function getFunctionReturnStatementNode(functionNode) {
    if (is_node_of_type_1.isBlockStatement(functionNode.body)) {
        const returnStatementNode = functionNode.body.body.find((statement) => is_node_of_type_1.isReturnStatement(statement));
...
function getPropertyIdentifierNode(node) {
    ...
    if (is_node_of_type_1.isMemberExpression(node)) {
        return getPropertyIdentifierNode(node.object);
    }
    if (is_node_of_type_1.isCallExpression(node)) {
        return getPropertyIdentifierNode(node.callee);
    }
    if (is_node_of_type_1.isExpressionStatement(node)) {
...
function getDeepestIdentifierNode(node) {
    ...
    if (is_node_of_type_1.isMemberExpression(node) && experimental_utils_1.ASTUtils.isIdentifier(node.property)) {
...
function getReferenceNode(node) {
    if (node.parent &&
        (is_node_of_type_1.isMemberExpression(node.parent) || is_node_of_type_1.isCallExpression(node.parent))) {
...
function getImportModuleName(node) {
    if (is_node_of_type_1.isImportDeclaration(node) && typeof node.source.value === 'string') {
        return node.source.value;
    }
    if (is_node_of_type_1.isCallExpression(node) &&
        is_node_of_type_1.isLiteral(node.arguments[0]) &&
...
function getAssertNodeInfo(node) {
    ...
    if (!is_node_of_type_1.isCallExpression(node.object) ||
    ...
    if (isNegated) {
        matcher =
            node.parent && is_node_of_type_1.isMemberExpression(node.parent)
...
function hasClosestExpectResolvesRejects(node) {
    if (is_node_of_type_1.isCallExpression(node) &&
        experimental_utils_1.ASTUtils.isIdentifier(node.callee) &&
        node.parent &&
        is_node_of_type_1.isMemberExpression(node.parent) &&
